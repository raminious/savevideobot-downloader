...
const getMedia = function* (id, format) {
  ...
  let media = yield cache.get(cache_key)

  if (media != null)
    return media

  media = yield findMediaById(id)
...
* get media thumbnail
*/
const getMediaThumbnail = function* (id) {

  // cache key
  const cache_key = 'media_thumb_' + id

  // check exists in cache
  const thumbnail = yield cache.get(cache_key)

  if (thumbnail != null)
    return thumbnail.url

  // get media from api server
  const media = yield findMediaById(id)
