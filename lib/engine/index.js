...
*/
const normalizers = {
  facebook: require('../normalizers/facebook')
}
...
/**
* normalize youtube-dl result for specific websites
*/
const normalize = function(response) {
  const { extractor } = response
  const normalizer = normalizers[extractor]
...
/*
* get media info from api server by id
*/
const findMediaById = async function (id) {

  try {
    // get media from api server
    return await Media.status(id)
  }
  catch(e) {
    throw {
      message: e.message || 'Internal server error. Downloader engine can not communicate with api server',
      description: e.response ? e.response.text : e.message,
...
/*
* get media info with format and its size
*/
const getMedia = async function (id, format) {

  format = format || 'best'

  //cache key
  const cache_key = 'media_' + id + '_' + format

  // get media from cache
  let media = await cache.get(cache_key)

  if (media != null)
    return media

  media = await findMediaById(id)
  ...
  // get media size
  if (size == null)
    size = await getMediaSize(download)

  // if file is less than 5k, throw error
  if (size < 5 * 1024) {
    throw {
      type: 'stream',
      message: 'File is too small or is stream (stream files are not supported)\n' + 'File size: ' + bytes(size)
    }
  }
  ...
  const response = {
    preferStream: true,
    id, format, site, title, filename, duration, url, thumbnail,
    download, stream, extension, size, worker
  }
...
*/
const getMediaSize = async function (url) {
  try {
...
* get media thumbnail
*/
const getMediaThumbnail = async function (id) {

  // cache key
  const cache_key = 'media_thumb_' + id

  // check exists in cache
  const thumbnail = await cache.get(cache_key)
  ...
  const isImage = ['jpg', 'jpeg', 'png', 'gif', 'webp'].indexOf(media.extension) > -1
  const thumbnailUrl = isImage ? media.download : media.thumbnail

  // store thumbnail in cache for 15 minutes
  cache.set(cache_key, { url: thumbnailUrl }, 15 * 60)
